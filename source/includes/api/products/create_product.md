...
The created product has no catalogue or category and a status of pending - this product needs to be approved in GPS before it will show in a catalogue
...
sku  | String  |  Required - cannonical unique reference or identity of a product used to for ordering
...
delivery_type_id  | Integer  |  Required - delivery types in GPS are (but not limited to) Physical = 1, Downloadable = 2, Eamil = 3, Prepaid = 4. These types are used to enforce validation strategies on orders for these products i.e. for physical orders, you must provide a postal address
